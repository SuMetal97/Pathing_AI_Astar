...
#Uses a heap to implement and store a priority queue
class PQ():
    def __init__(self):
        self.queue = []
        self.map = {}
...
class Map():
    ...
    #0 is straight line distance and 1 is fewest cities.
    ...
    #Calculation for a straight line distance
    def calc(self, a, b):
        loc1 = self.locate[a]
        x1 = int(loc1[0])
        y1 = int(loc1[1])
        ...
        if final_path == []:
            ...
            for x in range(0,len(final_path)-1):
                if heur == 0:
                    ...
                    total += dist
                    print(final_path[x] + " to " + final_path[x+1] + " length is %.1f" % dist)
                else:
                    total += 1
